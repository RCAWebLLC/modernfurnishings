...
AmazonListingProductSearchHandler.prototype = Object.extend(new CommonHandler(), {
                ...
                onSuccess: function (transport) {
                    ...
                    $('productSearch_cleanSuggest_button').observe('click', function() {
                        if (confirm(self.M2ePro.text.confirm)) {
                            popUp.close();
                            self.unmapFromAsin(productId, function() {
    ...
    showSearchManualPrompt: function ()
    {
        ...
        $('productSearch_submit_button').observe('click',function(event){
            self.searchAsinManual(self.params.productId);
        });
        ...
        $('query').observe('keypress',function(event) {
            event.keyCode == Event.KEY_RETURN && self.searchAsinManual(self.params.productId);
        });
    },

    showSearchAsinAutoPrompt: function ()
    {
        if (confirm(this.M2ePro.text.confirm)) {
            popUp.close();
            this.searchAsinAuto(this.params.productId);
        }
    },

    showUnmapFromAsinPrompt: function (productId)
    {
        MagentoMessageObj.clearAll();
        var self = this;

        if (confirm(self.M2ePro.text.confirm)) {
            this.unmapFromAsin(productId);
        }
    },
    ...
    searchAsinManual: function(productId)
    {
        var self = this;
    ...
    searchAsinAuto: function(productIds)
    {
        MagentoMessageObj.clearAll();
        var self = this;
    ...
    sendPartsOfProducts : function(parts,totalPartsCount)
    {
        ...
        ListingProgressBarObj.setStatus(str_replace('%execute%', partExecuteString, self.M2ePro.text.sending_data_message));
    ...
    mapToAsin: function(productId, generalId)
    {
        var self = this;
    ...
    unmapFromAsin: function(productIds, afterDoneFunction)
    {
        var self = this;
