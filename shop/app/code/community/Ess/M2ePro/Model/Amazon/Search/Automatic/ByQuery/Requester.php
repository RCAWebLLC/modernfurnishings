...
class Ess_M2ePro_Model_Amazon_Search_Automatic_ByQuery_Requester
{
    ...
    const STEP_TITLE_BY_TEMPLATE = 2;
    const STEP_TITLE_BY_PRODUCT = 3;
    ...
    public function isPossibleToSearch()
    {
        $result = !empty($this->currentQuery) &&
                  $this->currentStep <= self::STEP_TITLE_BY_PRODUCT;
    ...
    private function calculateCurrentData()
    {
        ...
        switch ($this->currentStep) {
            ...
            case self::STEP_TITLE_BY_TEMPLATE:

                    $tempQuery = $this->listingProduct->getChildObject()->getTitle();

                    !empty($tempQuery) && $this->currentStep = self::STEP_TITLE_BY_TEMPLATE;
                    !empty($tempQuery) && $this->currentQuery = (string)$tempQuery;

                break;

            case self::STEP_TITLE_BY_PRODUCT:

                    $tempQuery = $this->listingProduct->getMagentoProduct()->getName();

                    !empty($tempQuery) && $this->currentStep = self::STEP_TITLE_BY_PRODUCT;
                    !empty($tempQuery) && $this->currentQuery = (string)$tempQuery;
