...
class Ess_M2ePro_Adminhtml_Amazon_AccountController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    public function saveAction()
    {
        ...
        $model->getChildObject()->setSetting('other_listings_move_settings',
                                             array('synch','price'),
                                             $post['other_listings_move_synch_price']);
        $model->getChildObject()->setSetting('other_listings_move_settings',
                                             array('synch','qty'),
                                             $post['other_listings_move_synch_qty']);
        ...
        $extendedRoutersParams = array();
        $extendedRoutersParams['id'] = $id;

        $wizard = Mage::getSingleton('M2ePro/Wizard');
        $temp = Ess_M2ePro_Model_Wizard::STATUS_ACCOUNTS_AMAZON;
        if ($wizard->isUpgradeActive() &&
            $wizard->getStatus(Ess_M2ePro_Model_Wizard::MODE_UPGRADE) == $temp) {
            $extendedRoutersParams['hide_upgrade_notification'] = 'yes';
        }

        $this->_redirectUrl(Mage::helper('M2ePro')->getBackUrl('list',array(),array('edit'=>$extendedRoutersParams)));
    }
