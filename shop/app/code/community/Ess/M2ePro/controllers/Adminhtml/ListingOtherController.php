...
class Ess_M2ePro_Adminhtml_ListingOtherController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    protected function _initAction()
    {
        ...
        if (Mage::helper('M2ePro/Component_Amazon')->isActive()) {
            $this->getLayout()->getBlock('head')
                ->addJs('M2ePro/Amazon/Listing/Other/AutoMapHandler.js');
        }
    ...
    public function prepareMoveToListingAction()
    {
        ...
        foreach ($selectedProductsParts as $selectedProductsPart) {
            ...
            foreach ($tempData as $data) {
                $attributes[] = $data['attribute_set_id'];
            }
    ...
    public function unmapToProductAction()
    {
        ...
        if(!$listingOtherProductId) {
            ...
            return $this->_redirect('*/*/index/tab/'.$component);
        }
        ...
        Mage::getSingleton('core/resource')->getConnection('core_write')
            ->delete(Mage::getResourceModel('M2ePro/Amazon_Item')->getMainTable(),
                     array(
                         '`account_id` = ?' => $listingOtherProductInstance->getData('account_id'),
                         '`marketplace_id` = ?' => $listingOtherProductInstance->getData('marketplace_id'),
                         '`sku` = ?' => $listingOtherProductInstance->getChildObject()->getData('sku'),
                         '`product_id` = ?' => $listingOtherProductInstance->getData('product_id')
                     ));

        $listingOtherProductInstance->setData('product_id', NULL)->save();

        $logModel = Mage::getModel('M2ePro/Listing_Other_Log');
        $logModel->setComponentMode($listingOtherProductInstance->getComponentMode());
        $logModel->addProductMessage($listingOtherProductInstance->getId(),
        ...
        $this->_redirect('*/*/index/tab/'.$component);
    }
