...
HTML;
            ...
            $itemUrl = Mage::helper('M2ePro/Component_Ebay')->getItemUrl($item->getItemId(),
...
HTML;
    ...
    protected function callbackFilterEbayOrderId($collection, $column)
    {
        ...
        $collection->getSelect()->where('ebay_order_id LIKE ?', '%'.$value.'%');
    ...
    protected function callbackFilterItems($collection, $column)
    {
        ...
        $orderItemsCollection->getSelect()->where('item_id LIKE ?', '%'.$value.'%');
    ...
    protected function callbackFilterBuyer($collection, $column)
    {
        ...
        $collection->getSelect()->where('buyer_email LIKE ?', '%' . $value . '%');
        $collection->getSelect()->orWhere('buyer_user_id LIKE ?', '%' . $value . '%');
    ...
    protected function callbackFilterPaymentCondition($collection, $column)
    {
        ...
        if ($value == 1) {
            $this->getCollection()->addFieldToFilter('payment_status',
                                                     Ess_M2ePro_Model_Ebay_Order::PAYMENT_STATUS_COMPLETED);
        } else {
            $this->getCollection()
                          ->addFieldToFilter('payment_status',
    ...
    protected function callbackFilterShippingCondition($collection, $column)
    {
        ...
        if ($value == 1) {
            $this->getCollection()->addFieldToFilter('shipping_status',
                                                     Ess_M2ePro_Model_Ebay_Order::SHIPPING_STATUS_COMPLETED);
        } else {
            $this->getCollection()
                        ->addFieldToFilter('shipping_status',
    ...
    public function getRowUrl($row)
    {
        $back = Mage::helper('M2ePro')
                        ->makeBackUrlParam('*/adminhtml_order/index',
                                           array('tab' => Ess_M2ePro_Block_Adminhtml_Component_Abstract::TAB_ID_EBAY));

        $url = $this->getUrl('*/adminhtml_ebay_order/view',
                             array('id' => $row->getId(),
                                   'back'=>$back));
