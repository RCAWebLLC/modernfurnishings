...
class Ess_M2ePro_Block_Adminhtml_Amazon_Listing_View_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        //--------------------------------
        $marketplaceId = Mage::getResourceModel('core/config')->getReadConnection()
                               ->select()
                               ->from(Mage::getSingleton('core/resource')->getTableName('m2epro_template_general'),
                                      'marketplace_id')
                               ->where('id = ?', $listingData['template_general_id'])
                               ->where('component_mode = ?', $listingData['component_mode'])
                               ->query()
                               ->fetchColumn();
...
//        $withoutImageHtml = '<a href="'.$url.'">Map To Category</a><br>';
    ...
    public function callbackColumnDeveloperAction($value, $row, $column, $isExport)
    {
        ...
        $value != '' && $value .= '<br/>';
        $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                  .$row->getData('id').'); ListingActionHandlerObj.runStopAndRemoveProducts();">Remove</a>';
