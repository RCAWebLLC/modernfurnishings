...
class Ess_M2ePro_Block_Adminhtml_Amazon_Order_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('status', array(
            ...
            'type'    => 'options',
            'options' => array(
                Ess_M2ePro_Model_Amazon_Order::STATUS_PENDING             =>
                                                            Mage::helper('M2ePro')->__('Pending'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_UNSHIPPED           =>
                                                            Mage::helper('M2ePro')->__('Unshipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED_PARTIALLY   =>
                                                            Mage::helper('M2ePro')->__('Partially Shipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED             =>
                                                            Mage::helper('M2ePro')->__('Shipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_INVOICE_UNCONFIRMED =>
                                                            Mage::helper('M2ePro')->__('Invoice Not Confirmed'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_UNFULFILLABLE       =>
                                                            Mage::helper('M2ePro')->__('Unfulfillable'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_CANCELED            =>
                                                            Mage::helper('M2ePro')->__('Canceled')
            ),
...
HTML;
    ...
    {
        return Mage::helper('M2ePro/Currency')->formatPrice($row->getData('currency'), $row->getData('paid_amount'));
    }
    ...
    protected function callbackFilterItems($collection, $column)
    {
        ...
        $orderItemsCollection->getSelect()->orWhere('title LIKE ?', '%'.$value.'%');
        $orderItemsCollection->getSelect()->orWhere('sku LIKE ?', '%'.$value.'%');
        $orderItemsCollection->getSelect()->orWhere('general_id LIKE ?', '%'.$value.'%');
    ...
    public function getRowUrl($row)
    {
        ...
        return $this->getUrl('*/adminhtml_amazon_order/view',
                             array('id' => $row->getId(),
                                   'back'=>Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_order/index',
                                                                                    array('tab' => $tab))));
    }
