...
class Ess_M2ePro_Model_Magento_Quote_Item
{
    ...
    public function getProduct()
    {
        ...
        if ($this->product->getTypeId() == Ess_M2ePro_Model_Magento_Product::TYPE_GROUPED) {
    ...
    private function getAssociatedGroupedProduct()
    {
        ...
        $associatedProducts = Mage::getModel('M2ePro/Magento_Product')->setProduct($this->product)
    ...
    private function getProductTaxClassId()
    {
        ...
        if ($this->quoteBuilder->getProxyOrder()->isTaxModeNone()) {
            return Ess_M2ePro_Model_Magento_Product::TAX_CLASS_ID_NONE;
        }

        if ($this->quoteBuilder->getProxyOrder()->isTaxModeChannel() && $taxRate == 0) {
            return Ess_M2ePro_Model_Magento_Product::TAX_CLASS_ID_NONE;
        }

        if ($this->quoteBuilder->getProxyOrder()->isTaxModeMagento() ||
            $taxRate == 0 ||
