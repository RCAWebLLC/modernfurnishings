...
class Ess_M2ePro_Model_Ebay_Template_Description extends Ess_M2ePro_Model_Component_Child_Ebay_Abstract
{
    ...
    public function getWatermarkPriviousHash()
    {
        $settingNamePath = array(
            'hashes',
            'previous'
    ...
    public function addWatermarkIfNeed($imageLink)
    {
        ...
        $markingImagePath = $tempPath.'-m2epro-watermark-'.$this->getId().'-'.$this->getWatermarkHash().$fileExtension;
        ...
        $prevMarkingImagePath = $tempPath.'-m2epro-watermark-'.$this->getId();
        $prevMarkingImagePath .= '-'.$this->getWatermarkPriviousHash().$fileExtension;
