                                                                ...
                                                                Ess_M2ePro_Model_Amazon_Synchronization_Tasks
{
    ...
    private function execute()
    {
        ...
        $this->_lockItem->setPercents(self::PERCENTS_START + 1*self::PERCENTS_INTERVAL/6);
        $this->_lockItem->activate();

        $this->executePriceChanged();

        $this->_lockItem->setPercents(self::PERCENTS_START + 2*self::PERCENTS_INTERVAL/6);
        $this->_lockItem->activate();
        ...
        $this->_lockItem->setPercents(self::PERCENTS_START + 3*self::PERCENTS_INTERVAL/6);
        $this->_lockItem->activate();
        ...
        $this->_lockItem->setPercents(self::PERCENTS_START + 4*self::PERCENTS_INTERVAL/6);
        $this->_lockItem->activate();

        $this->executeDescriptionsTemplatesChanged();

        $this->_lockItem->setPercents(self::PERCENTS_START + 5*self::PERCENTS_INTERVAL/6);
        $this->_lockItem->activate();
    ...
    private function executeSpecialPriceIntervalChanged()
    {
        ...
        foreach ($changedListingsProducts as $changedListingProduct) {
            ...
            if ($this->_runnerActions
                     ->isExistProductAction(
                            ...
                            array('only_data'=>array('price'=>true,'variations'=>true)))
            ) {
                continue;
            }
        ...
        foreach ($changedListingsProductsVariationsOptions as $changedListingProductVariationOption) {
            ...
            if (!$listingProductVariationOption->getListingProduct()
                                               ->getMagentoProduct()
                                               ->isSimpleTypeWithCustomOptions() &&
                !$listingProductVariationOption->getListingProduct()
                                               ->getMagentoProduct()
                                               ->isGroupedType()
            ) {
            ...
            if ($attributeNeeded != 'special_price') {
                ...
                $src = $listingProductVariationOption->getSellingFormatTemplate()
                                                     ->getChildObject()
                                                     ->getSalePriceSource();
                if ($src['mode'] == Ess_M2ePro_Model_Amazon_Template_SellingFormat::PRICE_PRODUCT) {
                    $attributeNeeded = 'price';
            ...
            if (!$listingProductVariationOption->getSynchronizationTemplate()
                                               ->getChildObject()
                                               ->isReviseWhenChangePrice()
            ) {
                continue;
            }
